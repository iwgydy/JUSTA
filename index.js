...
let botCount = 0;
global.botSessions = {}; // เพื่อให้สามารถเข้าถึงได้ในคำสั่ง
const commands = {};
const commandDescriptions = [];
const commandUsage = {}; // ติดตามการใช้งานคำสั่ง
const autoReplyCommands = []; // เก็บคำสั่งที่เปิดใช้งานการตอบกลับอัตโนมัติ
    ...
    fs.readdirSync(commandsPath).forEach((file) => {
        if (file.endsWith(".js")) {
            ...
            if (command.config && command.config.name) {
                ...
                // ตรวจสอบว่าคำสั่งนี้เปิดใช้งานการตอบกลับอัตโนมัติหรือไม่
                if (command.config.autoReply) {
                    autoReplyCommands.push(command);
                    console.log(`🤖 เปิดใช้งานการตอบกลับอัตโนมัติสำหรับคำสั่ง: ${command.config.name}`);
                }
            }
        }
    });
    ...
    res.send(`
        ...
        <body>
            ...
            <script>
                ...
                // ฟังก์ชันสำหรับตอบกลับอัตโนมัติ
                async function handleAutoReplies(api, event) {
                    for (const command of autoReplyCommands) {
                        if (typeof command.runAutoReply === 'function') {
                            try {
                                await command.runAutoReply({ api, event, args: [] });
                                console.log(chalk.green(`✅ รันคำสั่งตอบกลับอัตโนมัติ: ${command.config.name}`));
                                // เพิ่มตัวนับการใช้คำสั่งตอบกลับอัตโนมัติ
                                ...
                                console.error(chalk.red(`❌ เกิดข้อผิดพลาดในคำสั่งตอบกลับอัตโนมัติ ${command.config.name}:`, error));
                            }
                        }
                    }
                }
            ...
            api.listenMqtt(async (err, event) => {
                ...
                if (event.type === "message") {
                    ...
                    if (message.startsWith(botSessions[token].prefix)) {
                        ...
                        // จัดการการตอบกลับอัตโนมัติสำหรับคำสั่งที่เปิดใช้งาน
