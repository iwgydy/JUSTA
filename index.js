...
const axios = require('axios'); // เพิ่มการนำเข้า axios
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                /* Additional Styles for AI Auto-Response */
                .ai-status {
                    margin-top: 10px;
                    font-size: 0.9rem;
                }
...
// หน้าเพิ่มบอท (ส่วน HTML เต็มๆ ถูกตัดออกเพื่อความกระชับ)

// หน้าแสดงบอทรัน (ส่วน HTML เต็มๆ ถูกตัดออกเพื่อความกระชับ)

// หน้าแสดงคำสั่งที่ใช้ (ส่วน HTML เต็มๆ ถูกตัดออกเพื่อความกระชับ)

// หน้า "วิธีทำบอทของคุณเอง" (ส่วน HTML เต็มๆ ถูกตัดออกเพื่อความกระชับ)
...
app.get("/debug/bots", (req, res) => {
    const bots = Object.entries(botSessions).map(([token, bot]) => ({
        ...
        ping: bot.ping || 'N/A',
        prefix: bot.prefix,
        aiEnabled: bot.aiEnabled || false // เพิ่มสถานะ AI
    }));
    res.json(bots);
});
...
async function startBot(appState, token, name, prefix, startTime, password, adminID, saveToFile = true) {
    return new Promise((resolve, reject) => {
        ...
        botSessions[token] = { 
            ...
            aiEnabled: false // เพิ่มตัวแปร aiEnabled เริ่มต้นเป็น false
        };
        ...
        login({ appState }, (err, api) => {
            ...
            api.listenMqtt(async (err, event) => {
                ...
                if (event.type === "message") {
                    ...
                    // ตรวจสอบว่าบอทตอบสนองต่อข้อความของตัวเองหรือไม่ เพื่อป้องกันลูปไม่รู้จบ
                    if (event.senderID === api.getCurrentUserID()) return;

                    // ตรวจสอบว่าบอทมีการตอบสนองอัตโนมัติเปิดอยู่หรือไม่
                    const bot = botSessions[token];
                    if (bot && bot.aiEnabled) {
                        try {
                            // ส่งข้อความไปยัง API
                            const response = await axios.get(`https://kaiz-apis.gleeze.com/api/claude-sonnet-3.5`, {
                                params: {
                                    q: message,
                                    uid: "1" // คุณสามารถปรับค่า uid ตามต้องการ
                                }
                            });

                            const aiReply = response.data.reply || "ไม่พบคำตอบจาก AI";

                            // ส่งข้อความตอบกลับ
                            bot.api.sendMessage(aiReply, event.threadID, event.messageID);
                        } catch (error) {
                            console.error(chalk.red(`❌ เกิดข้อผิดพลาดในการเรียก API: ${error.message}`));
                            bot.api.sendMessage("❗ เกิดข้อผิดพลาดในการประมวลผลคำขอของคุณ", event.threadID, event.messageID);
                        }
                    }

                    // ตัวอย่างคำสั่งที่มีคำนำหน้า
                    ...
                    if (command && typeof command.run === "function") {
                        try {
                            await command.run({ api, event, args, token });
                        ...
                        // ตัวเลือก: ตอบเมื่อไม่มีคำสั่งที่ตรงกัน
