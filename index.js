...
// เพิ่มตัวแปรสำหรับการตอบกลับอัตโนมัติ
global.autoReplyEnabled = false; // เริ่มต้นไม่เปิดใช้งานการตอบกลับอัตโนมัติ
    ...
    res.send(`
        ...
        <body>
            ...
            <script>
                ...
                // รับเหตุการณ์สำหรับการตอบกลับอัตโนมัติ
                socket.on('autoReplyMessage', (message, threadID) => {
                    if (global.autoReplyEnabled) {
                        autoReply(message, threadID);
                        showToast('ตอบกลับอัตโนมัติเรียบร้อยแล้ว', 'info');
                    }
                });

                // ฟังก์ชันตอบกลับอัตโนมัติ
                function autoReply(message, threadID) {
                    // ตัวอย่างการตอบกลับอัตโนมัติด้วยข้อความที่กำหนด
                    const replyMessage = "ขอบคุณที่ติดต่อเรา! เราจะตอบกลับเร็วๆ นี้.";
                    // ตรวจสอบว่าบอทที่เรียกใช้งานมี API
                    const bot = Object.values(botSessions).find(bot => bot.api);
                    if (bot && bot.api) {
                        bot.api.sendMessage(replyMessage, threadID);
                        console.log(chalk.green(`✅ ตอบกลับอัตโนมัติ: ${replyMessage}`));
                    } else {
                        console.error('❌ ไม่พบบอทที่สามารถส่งข้อความตอบกลับอัตโนมัติได้');
                    }
                }
        ...
        <body>
            ...
            <script>
                ...
                function autoReply(message, threadID) {
                    ...
                    if (bot && bot.api) {
                        bot.api.sendMessage(replyMessage, threadID);
                        console.log('✅ ตอบกลับอัตโนมัติ:', replyMessage);
                    } else {
                        console.error('❌ ไม่พบบอทที่สามารถส่งข้อความตอบกลับอัตโนมัติได้');
                    }
                }
                ...
                if (event.type === "message") {
                    ...
                    if (!message.startsWith(botSessions[token].prefix)) {
                        ...
                        if (global.autoReplyEnabled) {
                            ...
                            const bot = Object.values(botSessions).find(bot => bot.api);
                            if (bot && bot.api) {
                                bot.api.sendMessage(replyMessage, event.threadID);
                                console.log(chalk.green(`✅ ตอบกลับอัตโนมัติ: ${replyMessage}`));
                            } else {
                                console.error('❌ ไม่พบบอทที่สามารถส่งข้อความตอบกลับอัตโนมัติได้');
                            }
...
// Route สำหรับแสดงหน้าเพิ่มบอท (เสริมจาก app.get("/start") ที่มีอยู่แล้ว)
app.get("/start", (req, res) => {
    const error = req.query.error;
