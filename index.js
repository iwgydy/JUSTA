    ...
    res.send(`
        ...
        <body>
            ...
            <script>
                ...
                // รับเหตุการณ์อัปเดตบอทจากเซิร์ฟเวอร์
                socket.on('updateBots', (data) => {
                    const botTableBody = document.getElementById('botTableBody');
                    if (botTableBody) {
                        botTableBody.innerHTML = data.botRows;
...
// ฟังก์ชันบันทึกสถานะบอทลงไฟล์
const botStatusFile = path.join(__dirname, 'bots', 'botStatus.json');

// โหลดสถานะบอทจากไฟล์เมื่อเริ่มต้นเซิร์ฟเวอร์
if (fs.existsSync(botStatusFile)) {
    try {
        global.botStatus = JSON.parse(fs.readFileSync(botStatusFile, 'utf-8'));
        console.log(chalk.green('✅ โหลดสถานะบอทจากไฟล์เรียบร้อยแล้ว'));
    } catch (err) {
        console.error(chalk.red('❌ ไม่สามารถอ่านไฟล์สถานะบอท:', err));
        global.botStatus = {};
    }
} else {
    global.botStatus = {};
...
function saveBotStatus() {
    fs.writeFileSync(botStatusFile, JSON.stringify(global.botStatus, null, 4));
}
