 ...
 * ปรับปรุงจากโค้ดต้นฉบับ โดยเปลี่ยนให้เป็นธีมคริสต์มาส
 *****************************************************/
...
===========================
    ...
    // ─────────────────────────────────────────────────────────────────
    // ❸ กรณีเป็น "message_reply"
    // ─────────────────────────────────────────────────────────────────
    else if (event.type === "message_reply") {
        const replyMessage = event.body ? event.body.trim() : "";
        console.log(chalk.magenta(`🔃 มีคน reply ข้อความพร้อมเนื้อหา: "${replyMessage}" จากผู้ใช้ ${event.senderID}`));

        // ตัวอย่าง: เช็คว่าผู้ใช้ reply พร้อม prefix ไหม?
        if (replyMessage.startsWith(botSessions[token].prefix)) {
            // ถ้าจะให้คำสั่ง reply ได้ด้วย ก็เขียนเหมือนกรณี message
            const args = replyMessage
                .slice(botSessions[token].prefix.length)
                .trim()
                .split(/ +/);
            ...
            if (command && typeof command.run === "function") {
                try {
                    console.log(chalk.yellow(`🚀 (Reply) เรียกใช้คำสั่ง: ${commandName}`));
                    await command.run({ api, event, args });
                    console.log(chalk.green(`✅ (Reply) รันคำสั่งสำเร็จ: ${commandName}`));
                    ...
                    console.error(chalk.red(`❌ (Reply) เกิดข้อผิดพลาดในคำสั่ง ${commandName}:`, error));
                    api.sendMessage(
                        "❗ การรันคำสั่ง (reply) ล้มเหลว: " + error.message,
                        event.threadID
                    );
                }
            } else {
                api.sendMessage("❗ ไม่พบคำสั่ง (reply) ที่ระบุ", event.threadID);
            }
        } else {
            // ไม่ได้เป็นคำสั่ง → จะให้ทำอะไรก็ได้
            api.sendMessage("ฉันเห็นว่าคุณ reply ข้อความมา แต่ยังไม่ใช่คำสั่งนะ!", event.threadID);
        }
    }

    // ─────────────────────────────────────────────────────────────────
    // ❹ กรณีเป็น "message_reaction"
    // ─────────────────────────────────────────────────────────────────
    else if (event.type === "message_reaction") {
        console.log(chalk.magenta(`😆 มีคนกด reaction: ${event.reaction} โดยผู้ใช้ ${event.senderID}`));
        // ตัวอย่าง: จะทำอะไรก็ได้ เช่น ส่งข้อความตอบกลับ, เก็บสถิติ reaction ฯลฯ
        // api.sendMessage(`รับทราบว่า reaction ของคุณคือ ${event.reaction}`, event.threadID);
    }

    // ─────────────────────────────────────────────────────────────────
    // ❺ กรณีเป็น "read_receipt"
    // ─────────────────────────────────────────────────────────────────
    else if (event.type === "read_receipt") {
        console.log(chalk.magenta(`👀 ผู้ใช้อ่านข้อความแล้ว (threadID: ${event.threadID})`));
        // ตัวอย่าง: จะเก็บ log, ทำสถิติ หรือส่งข้อความแจ้งเตือนก็ได้
    }

    // ─────────────────────────────────────────────────────────────────
    // ❻ กรณีเป็น "typ" (กำลังพิมพ์)
    // ─────────────────────────────────────────────────────────────────
    else if (event.type === "typ") {
        console.log(chalk.magenta(`⌨️ ผู้ใช้ ${event.from} กำลังพิมพ์ข้อความ...`));
        // จะทำอะไรก็แล้วแต่ เช่นแจ้ง "บอทกำลังรออย่างใจจดใจจ่อ" ฯลฯ
    }

    // ─────────────────────────────────────────────────────────────────
    // ❼ กรณีอื่น ๆ ที่ยังไม่เคย handle
    // ─────────────────────────────────────────────────────────────────
    else {
        console.log(chalk.yellow(`❓ พบ event.type ที่ยังไม่ได้รองรับ: ${event.type}`));
        // จะลอง console.log ทั้ง event ออกมาดูก็ได้
        // console.log(event);
    }
});
