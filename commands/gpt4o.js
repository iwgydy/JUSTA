...
// โหลดอีเวนต์จากโฟลเดอร์ events (ถ้ามี)
const events = {};
...
// หน้าเพิ่มบอท (อีกครั้งเนื่องจากคำตอบก่อนหน้านี้ซ้ำกัน, กรุณาลบส่วนนี้ถ้ามีความซ้ำซ้อน)
app.get("/start", (req, res) => {
    const error = req.query.error;
...
async function startBot(appState, token, name, prefix, startTime, password, adminID, saveToFile = true) {
    return new Promise((resolve, reject) => {
        ...
        botSessions[token] = { 
            ...
            autoReply: false // เพิ่มสถานะ autoReply
        };
            ...
            api.listenMqtt(async (err, event) => {
                ...
                if (event.type === "message") {
                    ...
                    if (bot && bot.autoReply) {
                        // เรียกใช้ autoReplyHandler ของคำสั่ง gpt4o
                        const gpt4oCommand = commands['gpt4o'];
                        if (gpt4oCommand && typeof gpt4oCommand.autoReplyHandler === 'function') {
                            try {
                                await gpt4oCommand.autoReplyHandler({ api, event });
                                console.log(chalk.green(`✅ การตอบกลับอัตโนมัติผ่านคำสั่ง "gpt4o" ถูกเรียกใช้งาน`));
                                // เพิ่มตัวนับการใช้คำสั่ง
                                commandUsage['gpt4o'] = (commandUsage['gpt4o'] || 0) + 1;

                                io.emit('updateBots', generateBotData());
                                io.emit('updateCommands', generateCommandData());
                            } catch (error) {
                                console.error(chalk.red(`❌ เกิดข้อผิดพลาดในการตอบกลับอัตโนมัติผ่าน "gpt4o":`, error));
                                api.sendMessage("❗ การตอบกลับอัตโนมัติล้มเหลว", event.threadID);
                            }
                        }
                        return; // หยุดการทำงานเพื่อไม่ให้ตรวจสอบ prefix ต่อ
                    }
                    ...
                    if (message.startsWith(bot.prefix)) {
                        ...
                        if (command) {
                            try {
                                ...
                                console.error(chalk.red(`❌ เกิดข้อผิดพลาดในคำสั่ง "${command.config.name}":`, error));
...
// โหลดคำสั่ง gpt4o
const commandGPT4O = require('./commands/gpt4o');
commands[commandGPT4O.config.name.toLowerCase()] = commandGPT4O;

// ไม่จำเป็นต้องเพิ่มใน listener ของข้อความเพราะ autoReplyHandler จะถูกเรียกจาก event handler ของบอทเอง
